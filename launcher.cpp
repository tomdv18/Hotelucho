...
Launcher::Launcher(const std::string& nombreHotel): nombre(nombreHotel), hotel(nombre) {
    std::cout << BOLD
              << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
                 "~~~~~~~~~~~~~~~~~"
              << std::endl;
    std::cout << std::endl;
    std::cout << "~~~~~~~~~~~~ Bienvenido al sistema de gestion de hoteles "
                 "Hotelucho ~~~~~~~~~~~~~"
              << std::endl;
    std::cout << std::endl;
    std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
void Launcher::display() {
    header();
    std::cout << BOLD << BLUE << "Presiona 1 " << RESET
              << "para ver las habitaciones disponibles \n"
              << std::endl;  // READY
    std::cout << BOLD << BLUE << "Presiona 2 " << RESET
              << "para ver todas las habitaciones registradas\n"
              << std::endl;
...
void Launcher::pantalla_habitaciones_disponibles() {
    ...
    if (habitaciones.size() == 0) {
        ...
        std::cout << std::left << std::setw(ANCHO_PISO) << "\tPiso" << std::setw(ANCHO_NUMERO)
                  << "Numero" << std::setw(ANCHO_CAPACIDAD) << "Capacidad" << std::endl;
        std::string piso, numero, capacidad;
...
void Launcher::eliminar_habitacion() {
    ...
    solicitar_datos_habitacion(piso, numero);
    if (!hotel.existe_habitacion(numero, piso)) {
        std::cout << BOLD << "ERROR: No existe una habitacion registrada en ese piso y numero"
                  << RESET << std::endl;
        std::this_thread::sleep_for(std::chrono::seconds(2));
...
void Launcher::pantalla_habitacion(int numero, int piso) {
    ...
    if (info.dni_cliente != -1) {
        std::cout << std::left << "\tOcupante actual: " << info.nombre_cliente
                  << " (DNI: " << info.dni_cliente << ")" << std::endl;
    }
    std::cout << std::endl;
    std::cout << std::endl;
    char input;
...
void Launcher::pantalla_reserva() {
    ...
    if (!hotel.existe_habitacion(numero, piso)) {
        ...
        std::cout << BOLD << "Habitacion seleccionada con numero " << numero << " del piso " << piso
                  << std::endl;
        ...
        if (hotel.crear_reserva(numero, piso, stoi(dni), nombre, stoi(telefono), direccion) != 0) {
            ...
            std::cout << "Reserva realizada con exito" << std::endl;
        }
        std::this_thread::sleep_for(std::chrono::seconds(2));
    }
}
...
void Launcher::pantalla_habitaciones() {
    ...
    if (habitaciones.size() == 0) {
        ...
        std::cout << std::left << std::setw(ANCHO_PISO) << "\tPiso" << std::setw(ANCHO_NUMERO)
                  << "Numero" << std::setw(ANCHO_CAPACIDAD) << "Capacidad"
    ...
    char input;
    std::cout << BOLD << BLUE
              << "Agregar - 1\tEliminar - 2\tMantenimientos - 3\tInformacion - "
                 "4\nReservas - 5\tVolver - q"
              << RESET << std::endl;
    footer();
    std::cin >> input;
...
void Launcher::pantalla_clientes_registrados() {
    ...
    std::cout << std::endl;
    char input;
    std::cout << BOLD << BLUE << "Agregar - 1\tEliminar - 2\tModificar - 3\tVolver - q" << RESET
              << std::endl;
    footer();
    std::cin >> input;
    std::cin.ignore();
...
void Launcher::modificar_cliente() {
    ...
    bool cliente_encontrado = std::any_of(dnis_registrados.begin(), dnis_registrados.end(),
                                          [&](int id) { return stoi(dni) == id; });
    ...
    for (int id: dnis_registrados) {
        if (stoi(dni) == id) {
            if (!hotel.eliminar_cliente(stoi(dni))) {
                std::cout << "ERROR: El cliente actualmente esta registrado en una habitacion"
                          << std::endl;
                std::this_thread::sleep_for(std::chrono::seconds(2));
                return;
            }
